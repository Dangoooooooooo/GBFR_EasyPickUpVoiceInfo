...
#If the eof isn't provided, assume entire file is a bnk
    ...
    print("---BNK import finished.")
    return BNK

def writeBNK(BNK,exportDir):
    print("---BNK export started.")
    try:
...
def replaceWEM(MediaHeader,pData,WEM,needPadding):
    ...
    elif needPadding == False:
        removeByteSection(pData, MediaHeader["uOffset"], MediaHeader["uSize"])
        insertByteSection(pData, MediaHeader["uOffset"], WEM["data"])
...
def replaceBNKWEMs(BNK, inputDir,exportDir):#Edits the currently imported BNK
    if "DIDX" in BNK["chunkIndex"] and "DATA" in BNK["chunkIndex"]:
        ...
        for WEM in NewWEMList:   
            ...
            if BNK["chunkList"][chunkMediaIndex]["pLoadedMedia"][wemIndex]["id"] != BNK["chunkList"][chunkMediaIndex]["pLoadedMedia"][-1]["id"]:
                replaceWEM(BNK["chunkList"][chunkMediaIndex]["pLoadedMedia"][wemIndex],BNK["chunkList"][chunkDataIndex]["pData"],WEM,True)#Passes MediaHeader entry for specific wem
            else:
                replaceWEM(BNK["chunkList"][chunkMediaIndex]["pLoadedMedia"][wemIndex],BNK["chunkList"][chunkDataIndex]["pData"], WEM,False)
            newDataSize = len(BNK["chunkList"][chunkDataIndex]["pData"])
            sizeDifference = newDataSize - oldDataSize
            print ("WEM "+ str(WEM["id"])+" oldDataSize: "+str(oldDataSize)+" newDataSize: "+str(newDataSize)+" size difference: "+str(sizeDifference))
